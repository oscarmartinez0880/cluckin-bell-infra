...
# Dev+QA operator helper
# Usage:
#   PROFILE=cluckin-bell-qa REGION=us-east-1 CLUSTER=cluckn-bell-nonprod ./scripts/devqa-ops.sh up|open|status|down
#
...
wait_nodes_ready() {
  ...
  local tries=60
  until kubectl get nodes -o json 2>/dev/null | jq -e '[.items[] | select(any(.status.conditions[]; .type=="Ready" and .status=="True"))] | length >= 1' >/dev/null; do
    tries=$((tries-1))
    if [ $tries -le 0 ]; then
...
status() {
  ...
  log "Nodegroups:"
  for ng in $(list_nodegroups); do
    aws eks describe-nodegroup --profile "${PROFILE}" --region "${REGION}" --cluster-name "${CLUSTER}" --nodegroup-name "${ng}" \
...
open_tunnels() {
  ...
  if ! kubectl get ns monitoring >/dev/null 2>&1; then
    cat <<'EOF'
[INFO] Namespace 'monitoring' not found.
If you have not installed monitoring yet, run once:
  kubectl create namespace monitoring
  helm repo add prometheus-community https://prometheus-community.github.io/helm-charts
  helm repo update
  helm install kube-prometheus-stack prometheus-community/kube-prometheus-stack -n monitoring --set grafana.service.type=ClusterIP
Then re-run: ./scripts/devqa-ops.sh open
EOF
    # Don't exit hard; still try Argo CD below
  else
  ...
  # Argo CD
  if kubectl get ns argocd >/dev/null 2>&1 && kubectl -n argocd get svc argocd-server >/dev/null 2>&1; then
  ...
  # Keep process alive to hold tunnels
  log "Press Ctrl+C to close tunnels."
  wait
}

up()   { kube_login; scale_nodegroups 1 1 1; wait_nodes_ready; }
down() { kube_login; scale_nodegroups 0 0 1; log "Scaled to 0/0/1 (safe to shut down session)."; }
...
Environment overrides:
  PROFILE (default: cluckin-bell-qa)
  REGION  (default: us-east-1)
  CLUSTER (default: cluckn-bell-nonprod)
EOF
}

cmd="${1:-}"; case "${cmd}" in
  up) up ;; open) open_tunnels ;; status) status ;; down) down ;; *) usage; exit 1 ;;
esac