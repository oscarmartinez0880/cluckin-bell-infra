...
This Terraform configuration manages AWS infrastructure for the Cluckin Bell application across two AWS accounts with enhanced security and operability features.

## Infrastructure Improvements

- **Remote state**: S3 buckets per account (no DynamoDB). Bootstrap in terraform/state/bootstrap, then each stack uses s3 backend.
- **Default tags**: via provider.default_tags across stacks (Project, Owner, Env)
- **EKS managed add-ons**: vpc-cni, coredns, kube-proxy, aws-ebs-csi-driver
- **EKS secrets envelope encryption**: via KMS CMK (configurable)
- **Hardened controllers**: ALB Controller and ExternalDNS pinned versions, replicas, and PDBs
- **VPC endpoints**: for S3 and ECR to reduce NAT egress
- **EKS API endpoints**: private+public; public CIDRs configurable
- **OIDC trust**: limited to main branch and/or GitHub Environments (dev/qa/prod); optional workflow_ref pinning
...
### 1. Bootstrap state buckets (one-time):
```bash
...
### 2. Migrate stacks to S3 backend:
```bash
cd terraform/dns 
...
### 3. Re-apply stacks to roll out add-ons, encryption, endpoints, and hardened controllers.
...
- **EKS cluster**: cb-use1-shared with KMS encryption and managed add-ons
- **VPC endpoints**: S3 gateway + ECR interface to reduce NAT egress
- **Hardened controllers**: ALB Controller (v1.8.1) and ExternalDNS (v1.15.0) with HA
- **API access**: Private + public with configurable CIDRs

#### Production Cluster (`clusters/prod/`)
- **EKS cluster**: cb-use1-prod with KMS encryption and managed add-ons  
- **VPC endpoints**: S3 gateway + ECR interface to reduce NAT egress
- **Hardened controllers**: ALB Controller (v1.8.1) and ExternalDNS (v1.15.0) with HA
- **API access**: Private + public with configurable CIDRs

### Account-Level Resources (`accounts/`)
- **GitHub OIDC**: Tightened trust to protected refs and GitHub Environments
- **IAM roles**: For CI/CD with restricted conditions
...
Account-level IAM roles for GitHub Actions CI/CD have tightened trust policies:
...
**Note**: OIDC trust is now limited to main branch and GitHub Environments only.
...
- **KMS Keys**: `alias/cb-devqa-eks-secrets`, `alias/cb-prod-eks-secrets`

## Security Features

- **EKS encryption**: etcd secrets encrypted with customer-managed KMS keys
- **VPC endpoints**: S3 gateway + ECR interface reduce NAT gateway egress
- **OIDC restrictions**: GitHub trust limited to protected refs and environments  
- **Controller hardening**: Pinned versions, HA replicas, and Pod Disruption Budgets
- **Default tags**: Consistent tagging via provider configuration
